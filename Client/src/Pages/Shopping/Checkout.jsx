...
function Checkout() {
  ...
  useEffect(() => {
    if (imageCount === addToCartItems.length) {
      setImageLoad(false)
    }
  }, [imageCount])
  ...
  return (
    <>
      {(imageLoad ||addrLoad) && <Loader3/>}
      <div className={`min-h-screen bg-gray-50 ${(!loading && !imageLoad) ? "opacity-100" : "opacity-0"} transition-opacity duration-500`}>
