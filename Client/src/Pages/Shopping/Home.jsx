...
function Home() {
  ...
  const { data, category, featuredProductData, isLoading } = useSelector(state => state.homeData)
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [imageCount, serImageCount] = useState(0)
  ...
  return (
    <>
      {(loading || isLoading) && <Loader3 loading />}
      <div className={`mb-15 ${loading ? "opacity-0" : "opacity-100"} transition-opacity duration-500`}>
          ...
          <div className='grid sm:grid-cols-[repeat(auto-fill,minmax(130px,1fr))] grid-cols-4 gap-3' >
            {category?.map(item =>
              <div onClick={() => navigate(`/shop/lists/${item._id}`)} key={item._id} className='p-3 rounded-md bg-white cursor-pointer'>
                <img loading='lazy' onLoad={() => serImageCount(prev => prev + 1)} src={item.image} className='rounded-md' />
